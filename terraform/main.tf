...
resource "aws_autoscaling_group" "wp_asg" {
  name                      = "wp_asg-${aws_launch_configuration.wp_lc.id}"
...
resource "aws_route53_record" "www" {
  ...
  name    = "www.${var.domain_name}.${var.domain_ending}"
  type    = "A"
...
resource "aws_route53_record" "dev" {
  zone_id = "${aws_route53_zone.primary.id}"
  name    = "dev.${var.domain_name}.${var.domain_ending}"
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.wp_dev.public_ip}"]
}

# Private zone (secondary)
...
resource "aws_route53_record" "db" {
  ...
  name    = "db.${var.domain_name}.${var.domain_ending}"
  type    = "CNAME"
  ttl     = "300"
